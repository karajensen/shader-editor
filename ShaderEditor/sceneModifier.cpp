...
void SceneModifier::UpdateWater()
{
    ...
    if(selectedWater != m_selectedWater)
    {
        ...
        m_cache->Water[WATER_DEEP].SetUpdated(water.deepColour);
        m_cache->Water[WATER_SHALLOW].SetUpdated(water.shallowColour);
    ...
    else if(m_selectedWater >= 0 && m_selectedWater < m_scene.GetWaterCount())
    {
        ...
        water.deepColour = m_cache->Water[WATER_DEEP].Get();
        water.shallowColour = m_cache->Water[WATER_SHALLOW].Get();
